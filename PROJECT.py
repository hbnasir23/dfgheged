...
root=Tk(className=" Health Tracker & Workout Generator!")
# tk_image = PhotoImage(file=r"C:\Users\hbnas\OneDrive\Documents\GitHub\health_tracker_and_recommender\images.png")
...
def menu_about():
    response=messagebox.showinfo("About us","Developed by : Haris Bin Nasir(SE-084), Ammar Bin Amir(SE-098), Maaz Ali Murtaza(SE-092) \nContact us:  \nE-mail: neduet-se84,92,98@gmail.com \nPhone: +92 331811583, +92 3232777244, +92 3312873585  \nGet more info at www.TrackHealthandfitnessgenerator.pk")

def menu_new_file():
    for widget in panel_2.winfo_children():
       widget.destroy()
    check_button=Button(panel_2,text="Click here to Enter your Information",bg="white",padx=5,pady=10,command=m_window)
    check_button.pack(anchor=LEFT,pady=10)

def submit_fn():
    messagebox.askokcancel("askokcancel", "Want to continue?")

#creating Frame root
frame_root=Frame(root,bg="white")
frame_root.pack()
...
panel_2=Frame(panel_1,relief=FLAT,bg="crimson")
panel_1.add(panel_2)
...
def m_window():
    ...
    Label(panel_2,text="\nEnter your Bio Data...",font=30,bg="sky blue").pack(anchor=N)
    Label(panel_2, text="\nTime:",font=35, bg="light green").pack(anchor=NE)
    Label(panel_2, text=x,bg="sky blue").pack(anchor=NE)

    Label(panel_2, text="\nFirst Name:",bg="sky blue").pack()
    f_name=Entry(panel_2,border=5,width=30)
    f_name.pack()
    Label(panel_2, text="Last Name:",bg="sky blue").pack()
    l_name=Entry(panel_2,border=5,width=30)
    l_name.pack()

    var=IntVar()
    male=Radiobutton(panel_2,text="Male",bg="sky blue",variable=var,value=True,command=None)
    male.pack()
    female=Radiobutton(panel_2,text="Female",bg="sky blue",variable=var,value=False,command=None)
    female.pack()

    Label(panel_2, text="Age:",bg="sky blue").pack()
    age=Entry(panel_2,border=5,width=30)
    age.pack()
    Label(panel_2, text="Address:",bg="sky blue").pack()
    address=Entry(panel_2,border=5,width=30)
    address.pack()
    Label(panel_2, text="Contact No:",bg="sky blue").pack()
    contact=Entry(panel_2,border=5,width=30)
    contact.pack()
    #submit button
    submit_button=Button(panel_2,text="Submit",relief=RAISED,padx=30,pady=10,font="Algerian",bg="sky blue",command=submit_fn)
    submit_button.pack()
    #check yourself button
    check_button=Button(panel_2,text=" Check Yourself",relief=RAISED,bg="sky blue",padx=30,pady=10,font="Algerian",command=check_health_alloption)
    check_button.pack()

m_window_canvas=Canvas(panel_2,height=600,width=500,bg="#753a88",relief=RAISED)
m_window_canvas.pack(expand=YES,fill=BOTH,side="left")
gig = PhotoImage(file=r'C:\Users\hbnas\OneDrive\Desktop\NEWWWWWWLOGOOOOO.png')


button1=Button(m_window_canvas,text="Click here to Enter your Information",bg="#3FEBD8",font=8,relief="raised",padx=5,pady=10,command=m_window)
button1.place(relx=0.5, rely=0.65, anchor="center")
m_window_canvas.create_image(850,260,image=gig)
...
def check_health():
    ...
    c=float(a * b)
    bmi = b/(a*a)
    Label(check_health_canvas,text ="Generated Report\n",font=20,bg="teal").pack(side="top")
    Label2=Label(check_health_canvas)
    if ( bmi < 16):
        Label2.configure(text = "\nPhysical Status: Severely Underweight",font=18,bg="teal")

    elif ( bmi >= 16 and bmi < 18.5):
        Label2.configure(text ="\nPhysical Status: Underweight",font=18,bg="teal")

    elif ( bmi >= 18.5 and bmi < 25):
        Label2.configure(text ="\nPhysical Status: Healthy",font=18,bg="teal")

    elif ( bmi >= 25 and bmi < 30):
        Label2.configure(text = "\nPhysical Status: Overweight",font=18,bg="teal")

    elif ( bmi >=30):
        Label2.configure(text ="\nPhysical Status:Severely Overweight",font=18,bg="teal")

    Label1=Label(check_health_canvas)
    b=('BMI Index')

    a=(b,bmi)
    Label1.configure(text = a,font=18,bg="teal")
    ...
    Label1=Label(check_health_canvas)
    if sugar>100:
          Label1.configure(text="\nSugar Level : High.",font=18,bg="teal")
    elif sugar<60:
        Label1.configure(text="\nSugar Level : Low.",font=18,bg="teal")
    else:
        Label1.configure(text="\nSugar Level : Fine.",font=18,bg="teal")
    Label1.pack(anchor=NW)
    ...
    def high_bp(up,down):
                ...
                if((int(bp_high)<130 and int(bp_high)>=120 )and int(bp_low)<=80):
                    bph_Label.configure(text="\nyou area in PREHYPERTENTION STAGE",font=18,bg="teal")
                elif((int(bp_high)<140 and int(bp_high)>=130) and int(bp_low)<90):
                    bph_Label.configure(text="\nYou are in HYPERTENTION STAGE 1",font=18,bg="teal")
                elif((int(bp_high)<180 and int(bp_high)>=140) and int(bp_low)<120):
                    bph_Label.configure(text="\nYou are in HYPERTENTION STAGE 2",font=18,bg="teal")
                else:
                    bph_Label.configure(text="\nYou are in HYPERTENSIVE CRISIS(Final stage of HYPERTENTIOIN",font=18,bg="teal")
    ...
    if(up<90 and up>=40 and down<60 and down>=40):
        ...
        bp_Label.configure(text="\nPlease enter a valid blood pressure",font=18,bg="teal")
...
def check_health_alloption():
    ...
    Label(panel_2,text="Track your Health Status...",font=("Georgia",40),bg="crimson").pack(anchor=N)

    #canvas inside panel_2 for report generatioin
    global check_health_canvas
    check_health_canvas=Canvas(panel_2,bg="teal",width=500,height=400)
    check_health_canvas.pack(fill=BOTH,side="right")

    #BMI
    global height
    Label(panel_2, text="BMI Calculator", font=("Ink Free", 20, "bold"), bg="crimson").pack(anchor=NW)

    Label(panel_2, text="\nHeight (metre):",bg="crimson").pack(anchor=NW)
    height = Entry(panel_2,border=5,width=30)
    height.pack(anchor=NW)
    global weight
    Label(panel_2, text="Weight (Kg):",bg="crimson").pack(anchor=NW)
    ...
    global sugar_level
    Label(panel_2,text="\nSugar Level Calulator",font=("Ink Free", 20, "bold"),bg="crimson").pack(anchor=NW)
    Label(panel_2, text="\nSugar Level:",bg="crimson").pack(anchor=NW)
    ...
    Label(panel_2,text="\nBlood Pressure Calulator",font=("Ink Free", 20, "bold"),bg="crimson").pack(anchor=NW)
    Label(panel_2,text="\nBP(Upper):",bg="crimson").pack(anchor=NW)
    bp_high=Entry(panel_2,border=5,width=30)
    bp_high.pack(anchor=NW)
    global bp_low
    Label(panel_2,text="BP(Lower)",bg="crimson").pack(anchor=NW)
...
def workout():
    for widget in cnvs.winfo_children():
        widget.destroy()
    Label4 = Label(cnvs, text='', bg="white", font=40)
    Label4.pack(pady=20)
    temp = sp.get()
    if (temp == 'Dengue'):
        Label4.configure(
            text="***Dengue home treatment***\n\n1.Papaya Leaf Juice\n2.Goat Milk\n3.Giloy Juice\n4.Guava Juice\n5.Apple Juice\n6.Neem Leaves\n7.Kiwi Juice\n8.Vitamin K\n9.Turmeric\n10.Tulsi Leaves")
    elif (temp == "Malaria"):
        Label4.configure(
            text="***Malaria home treatment***\n\n1.Orange Juice\n2.Ginger\n3.Lime or lemon Juice\n4.Holy Basil\n5.Warm Water\n6.Grapeffruit Juice\n7.Berries")
    elif (temp == "Corona"):
        Label4.configure(
            text="***Corona home treatment***\n\n1.Warm water\n2.Giloy\n3.Tulsi(Basil)\n4.Turmeric powder\n5.Vitamin C rich foods(Kiwi,orange,papaya...)")
    elif (temp == "Typhoid"):
        Label4.configure(
            text="***Typhoid home treatment***\n\n1.Drink lots of fluids\n2.Garlic\n3. Basil\n4.Apple cider vinegar\n5.Cold compress")
    elif (temp == "Common Cold"):
        Label4.configure(
            text="***Common Cold home treatment***\n\n1.Garlic\n2.Raw Honey\n3.Ginger\n4.Chicken Soup\n5. Red Onion\n6.Black Pepper\n7.Turmeric Milk")
    else:
        Label4.configure(
            text="***Allergies home treatment***\n\n1.Use HEPA filters\n2.Butterbur\n3.papaya and pineapple\n4.Honey\n5.Vitamin C")
...
#Calling workout
    disease_compute=Button(panel_2,text="Check",border=1,relief="raised",bg="white",font=20,command=other_diseases)
    disease_compute.pack(pady=30)

    cnvs=Canvas(panel_2,width=0,height=0,bg="blue")
    cnvs.pack(expand=YES,fill=BOTH,padx=20)
...
gig1 = PhotoImage(file=r'C:\Users\hbnas\OneDrive\Desktop\GDHELTH24.png')
